...
pub fn generate_mesh<'a, V: RectRasterableVol<Vox = Block> + ReadVol + Debug>(
    vol: &'a VolGrid2d<V>,
    (range, max_texture_size): (Aabb<i32>, Vec2<u16>),
) -> MeshGen<TerrainVertex, FluidVertex, TerrainVertex, (Aabb<f32>, ColLightInfo)> {
    ...
    let mut light = calc_light(range, vol, lit_blocks);
