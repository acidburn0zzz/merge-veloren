...
pub fn generate_mesh_base_vol_terrain<'a: 'b, 'b, V: 'a>(
    vol: V,
    (greedy, opaque_mesh, offs, scale): (
        &'b mut GreedyMesh<'a>,
        &'b mut Mesh<TerrainVertex>,
        Vec3<f32>,
        Vec3<f32>,
    ),
) -> MeshGen<TerrainVertex, TerrainVertex, TerrainVertex, (math::Aabb<f32>, Range<usize>)>
where
    V: BaseVol<Vox = Cell> + ReadVol + SizedVol,
{
    ...
    let create_opaque =
        |atlas_pos, pos, norm| TerrainVertex::new_figure(atlas_pos, (pos + offs) * scale, norm, 0);
